from .utils.error import FileError, FlagError
import sys
...
class CommandlineHandler():
    ...
    def handle_args(self):
        if self.argv == []:
            self.error = FileError("InvalidFileError", "No file is specified!", "")
            return None, None
        if self.argv[0] == "--help":
            self.error = HELP_MSG
            return None, None
        try:
            ...
            self.error = FileError("FileNotFoundError", "The given file was not found or has an invalid file extension", self.argv[-1])
            return None, None
        if len(self.argv) == 1:
            return self.argv[0], {}
